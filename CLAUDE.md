...
**Branch:** `bedrock-version`
**Status:** Production-ready pipeline with systematic experiment-backed configuration
...
```
Phase 1: 50 candidates × tag metrics (0 API calls)
    ↓ top 10
Phase 2: LLM coherence check (~10×k Claude calls)
    ↓ re-rank by 40% LLM + 60% tags
    ↓ top 3
Phase 3: Stage 2 sub-clustering + L1 consolidation
    ↓ best result
Stage 3: Optional L3 splits
    ↓
Orphan reassignment (misplaced techs)
    ↓
Small leaf merging (≤2 tech leaves)
    ↓
Excel output
```
...
### Naming Pitfalls
- Claude tends to use generic words (Frontier, Emerging, Advanced) — blocked via forbidden list
- Without `existing_names`, Claude gives identical names to different clusters
- Both issues are now handled in the `name_cluster` function
...
| File | Purpose |
|------|---------|
| `hierarchical_taxonomy.py` | Production pipeline — Cohere v4 + multi-algo + LLM coherence |
| `config.py` | AWS credentials (gitignored) |
| `config.example.py` | Template for credentials |
| `list-main-tech.csv` | Input data (202 technologies) |
...
Three refinement steps run after the main clustering pipeline:

### 1. L1 Consolidation (`consolidate_l1_groups`)
- **When:** After Stage 2, inside correction loop
- **What:** Merges L1 groups whose mean embeddings have cosine similarity > 0.80
- **Why:** Prevents fragmentation (e.g., 4 separate bio-energy groups → 2)
...
- **What:** Merges leaf clusters with ≤2 techs into their most similar sibling under the same parent
